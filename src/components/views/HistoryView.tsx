            ...
            <div className="space-y-0">
                {history.length > 0 ? history.map((h) => (
                    <div key={h.id} className="timeline-line relative pl-6 pb-6 last:pb-0">
                        <div className={`absolute left-0 top-1.5 w-3 h-3 rounded-full border-2 border-skin-card ${h.action === '解約' || h.action === 'Canceled' ? 'bg-rose-400' : 'bg-emerald-400'}`}></div>
                        <p className="text-[10px] text-skin-subtext font-bold mb-0.5">{h.date}</p>
                        <p className="text-sm font-bold">{h.subName} <span className="text-xs font-normal text-skin-subtext">- {t(`history.labels.${(h.action === '解約' || h.action === 'Canceled') ? 'cancel' : (h.action === '再契約' || h.action === 'Resumed') ? 'resume' : 'new'}`) || h.action}</span></p>
                    </div>
